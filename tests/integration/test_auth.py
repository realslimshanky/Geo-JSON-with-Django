...
from geo_json_with_django.users.auth.tokens import get_token_for_password_reset
...
def test_user_password_reset_and_confirm(client, settings, mocker):
    url = reverse('auth-password-reset')
    user = f.create_user(email='test@example.com')
    mock_email = mocker.patch('geo_json_with_django.users.auth.services.send_mail')
